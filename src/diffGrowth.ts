import { PI, rndm, Vec, Vp } from '@rupertofly/h';
...
const MAX_FORCE = 0.2;
const MAX_SPEED = 0.6;
const DESIRED_SEPARATION = 10;
const COHESION_RATIO = 1.1;
const MAX_EDGE_LEN = 8;
const DES_SEP_SQ = Math.pow(DESIRED_SEPARATION, 2);
function search(quadtree: Quadtree<Node>, xmin: number, ymin: number, xmax: number, ymax: number) {
  const results: Node[] = [];
  ...
  sepparate(nodes: Array<Node>): Vec {
    ...
    for (let other of nodes) {
      ...
      if (dist > 0 && dist < DES_SEP_SQ) {
        let diff = this.position.clone().sub(other.position).norm().divScaler(Math.sqrt(dist));
        steer.add(diff);
        count++;
      }
    }
